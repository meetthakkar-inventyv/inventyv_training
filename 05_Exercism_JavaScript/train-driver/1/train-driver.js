...
export function fixListOfWagons(ids) {
  const [firstEle, secondEle, ...rest] = ids;
  return [...rest, firstEle, secondEle];
}
...
export function correctListOfWagons(ids, missingWagons) {
   const [firstEle, ...rest] = ids;
  return [firstEle, ...missingWagons, ...rest];
}
...
export function extendRouteInformation(information, additional) {
  const mergedObj = {...information , ...additional};
  return mergedObj;
}
